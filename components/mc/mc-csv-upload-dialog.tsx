...
interface MCCSVUploadDialogProps {
  onUpload: (data: MCRecord[], fileName: string) => Promise<void>;
  companyId: string;
  profileId: string;
}

export function MCCSVUploadDialog({ onUpload, companyId, profileId }: MCCSVUploadDialogProps) {
  const [open, setOpen] = useState(false);
  const [file, setFile] = useState<File | null>(null);
  const [previewData, setPreviewData] = useState<MCRecord[]>([]);
  ...
  const filterCSVData = (csvData: any[], csvHeaders: string[]): MCRecord[] => {
    ...
    // Filter data to only include required columns and rows with study visit
    return csvData
      .map((row) => {
        const filteredRow: MCRecord = {};
        REQUIRED_COLUMNS.forEach((col) => {
      ...
      .filter((row) => {
        // Filter out rows without a study visit value
        const studyVisit = row["1.CCSVT"];
        return studyVisit && studyVisit.trim() !== "";
      });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-[700px]">
        <DialogHeader>
          <DialogTitle className="text-lg">Upload Medication Compliance CSV</DialogTitle>
          <DialogDescription className="text-xs">
            Upload a medication compliance CSV file. Only columns matching the required headers (row 2) will be displayed.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          >
            ...
            {!file ? (
              <label htmlFor="mc-csv-upload" className="cursor-pointer">
