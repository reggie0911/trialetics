...
function makeContact(over: Partial<DirectoryContactListItem> & { id: string; first_name: string; last_name: string }): DirectoryContactListItem {
  return {
    ...
    address_line1: over.address_line1 ?? null,
    city: over.city ?? null,
    postal_code: over.postal_code ?? null,
    contact_address_source: over.contact_address_source ?? 'manual',
    contact_address_study_site_id: over.contact_address_study_site_id ?? null,
