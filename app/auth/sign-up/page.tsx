    ...
    try {
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          ...
          emailRedirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent(planParam ? `/pricing?checkout=1&plan=${planParam}&interval=${intervalParam}` : '/pricing')}`,
        },
      });
      ...
      if (data.session) {
        ...
        // If the user came from a specific pricing plan, go straight to checkout.
        if (planParam) {
          try {
      ...
      // Email confirmation on: send to confirmation waiting page.
      // Pass plan params so they can be resumed after email confirm.
      const next = planParam
        ? `/pricing?checkout=1&plan=${planParam}&interval=${intervalParam}`
        : '/pricing';
