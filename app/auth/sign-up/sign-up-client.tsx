...
function SignUpForm() {
  ...
  useEffect(() => {
    if (!planParam) {
      router.replace('/pricing?intent=signup');
    }
  }, [planParam, router]);
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (!planParam) {
      setError('Please choose a plan on the pricing page first.');
      setIsLoading(false);
      return;
    }
    ...
    try {
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          ...
          emailRedirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent(`/pricing?checkout=1&plan=${planParam}&interval=${intervalParam}`)}`,
        },
      });
      ...
      if (data.session) {
        ...
        try {
          const delaysMs = [0, 400, 800, 1500, 2500];
          let checkoutData: { url?: string; error?: string; detail?: string } | null = null;
          ...
          for (const waitMs of delaysMs) {
            ...
            lastRes = res;
            checkoutData = (await res.json()) as {
              url?: string;
              error?: string;
              detail?: string;
            };
            ...
            const retryable =
              res.status === 503 ||
              checkoutData.error === 'No company found' ||
              (typeof checkoutData.error === 'string' &&
                checkoutData.error.includes('still initializing'));
            if (!retryable) break;
          }

          if (checkoutData?.detail) {
            console.error('[sign-up] checkout failed', checkoutData);
          }
          ...
          if (lastRes && !lastRes.ok) {
            setError('Could not start checkout. Try again from Pricing or Billing.');
            setIsLoading(false);
            return;
          }
          setError('Could not start checkout. Redirecting to pricing to try again…');
          router.push(`/pricing?checkout=1&plan=${planParam}&interval=${intervalParam}`);
        } catch {
          setError('Could not reach billing. Redirecting to pricing…');
          router.push(`/pricing?checkout=1&plan=${planParam}&interval=${intervalParam}`);
        }
        setIsLoading(false);
        return;
      ...
      const next = `/pricing?checkout=1&plan=${planParam}&interval=${intervalParam}`;
