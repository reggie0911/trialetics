...
interface StudyEisfFolderPageProps {
  params: Promise<{ id: string; folderId: string }>;
}

export default async function StudyEisfFolderPage({ params }: StudyEisfFolderPageProps) {
  const { id: studyId, folderId } = await params;
  const folderRes = await getEisfFolder(folderId);
  if (!folderRes.success || !folderRes.data || folderRes.data.study_id !== studyId) notFound();

  const [docsRes, reqRes, catRes] = await Promise.all([
    listEisfDocuments(folderId),
    listEisfRequests({ folderId, studyId }),
    getEisfCategories(),
  ]);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <Button asChild variant="outline" size="sm" className="text-[12px]">
            <Link href={`/protected/studies/${studyId}/eisf/folders`}>All folders</Link>
          </Button>
        </div>
      </div>
