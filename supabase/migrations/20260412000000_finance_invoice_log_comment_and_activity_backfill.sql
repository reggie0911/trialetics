-- Optional follow-up: include decision comment in finance_transaction_log.payload (approve/reject),
-- and backfill synthetic draft_created / submitted rows for existing invoices (idempotent).
...
DECLARE
  ...
  v_comment jsonb;
BEGIN
  v_comment := to_jsonb(NULLIF(trim(COALESCE(p_comment, '')), ''));
...
INSERT INTO public.finance_transaction_log (
  company_id,
  study_id,
  entity_type,
  entity_id,
...
WHERE NOT EXISTS (
  SELECT 1
  FROM public.finance_transaction_log l
  WHERE l.entity_type = 'finance_invoice'
    AND l.entity_id = fi.id
    AND l.action = 'draft_created'
);
...
SELECT
  ...
  'draft',
  CASE
    WHEN fi.status IN ('submitted', 'under_review') THEN fi.status::text
    ELSE 'under_review'
  END,
  jsonb_build_object(
    'backfill', true,
    'note', 'Approximate: true submit time was not stored; timestamp uses received_at vs created_at heuristic.'
  ),
  CASE
    WHEN fi.received_at > fi.created_at THEN fi.received_at
    ELSE fi.created_at + interval '1 second'
  END
FROM public.finance_invoices fi
WHERE fi.status IS DISTINCT FROM 'draft'
  AND NOT EXISTS (
    SELECT 1
    FROM public.finance_transaction_log l
    WHERE l.entity_type = 'finance_invoice'
