-- =============================================
-- Update Visit Templates with Working Schema
-- Replaces broken templates with minimal working schema
-- =============================================

-- Delete existing templates to replace them with working versions
DELETE FROM public.template_activities WHERE template_visit_id IN (
  SELECT tv.id FROM public.template_visits tv
  JOIN public.subject_visit_templates t ON tv.template_id = t.id
  WHERE t.company_id = '397cadc7-e336-4497-ae17-6ec178de33c1'
);

DELETE FROM public.template_visits WHERE template_id IN (
  SELECT id FROM public.subject_visit_templates
  WHERE company_id = '397cadc7-e336-4497-ae17-6ec178de33c1'
);

DELETE FROM public.subject_visit_templates 
WHERE company_id = '397cadc7-e336-4497-ae17-6ec178de33c1';

-- Now insert working templates
DO $$
DECLARE
...
BEGIN
  ...
  RAISE NOTICE 'SUCCESS: Templates updated for company %', v_company_id;
END $$;
