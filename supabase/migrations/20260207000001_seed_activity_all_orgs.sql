-- =============================================
-- Seed Activity Data for ALL Site Organizations
-- Adds realistic sample activity history for all sites
-- =============================================
...
BEGIN
  ...
  RAISE NOTICE 'Seeding activity data for all site organizations in company: %', v_company_id;

  -- Loop through all site organizations
  FOR v_org_id IN 
  ...
  LOOP
    ...
    RAISE NOTICE 'Adding activity data for organization #%: %', v_org_count, v_org_id;

    -- Delete any existing activities for this org to avoid duplicates
    DELETE FROM public.organization_activity WHERE organization_id = v_org_id;

    -- Activity 1: Organization created (15 days ago)
    INSERT INTO public.organization_activity (
      organization_id, activity_type, description, changed_fields,
      performed_by_id, performer_email, created_at
    ) VALUES (
      v_org_id, 'created', 'Site organization created', '{}'::jsonb,
      v_profile_id, v_creator_email, NOW() - INTERVAL '15 days'
    );
    ...
    INSERT INTO public.organization_activity (
      ...
      v_org_id, 'updated', 'Updated contact information',
      jsonb_build_object(
      ...
      v_profile_id, v_creator_email, NOW() - INTERVAL '12 days'
    );
    ...
    INSERT INTO public.organization_activity (
      ...
      v_org_id, 'updated', 'Updated primary address',
      jsonb_build_object(
      ...
      v_profile_id, v_creator_email, NOW() - INTERVAL '10 days'
    );
    ...
    INSERT INTO public.organization_activity (
      ...
      v_org_id, 'updated', 'Assigned Principal Investigator',
      jsonb_build_object(
        'principal_investigator', jsonb_build_object('old', 'N/A', 'new', 'Dr. Sarah Mitchell')
      ),
      v_profile_id, 'john.admin@trialetics.com', NOW() - INTERVAL '8 days'
    );
    ...
    INSERT INTO public.organization_activity (
      ...
      v_org_id, 'updated', 'Updated site milestones',
      jsonb_build_object(
      ...
      v_profile_id, 'jane.monitor@trialetics.com', NOW() - INTERVAL '7 days'
    );
    ...
    INSERT INTO public.organization_activity (
      ...
      v_org_id, 'status_changed', 'Site status changed to active',
      jsonb_build_object(
        'status', jsonb_build_object('old', 'pending', 'new', 'active')
      ),
      v_profile_id, v_creator_email, NOW() - INTERVAL '5 days'
    );
    ...
    INSERT INTO public.organization_activity (
      ...
      v_org_id, 'updated', 'Updated IRB information',
      jsonb_build_object(
      ...
      v_profile_id, 'compliance@trialetics.com', NOW() - INTERVAL '3 days'
    );
    ...
    INSERT INTO public.organization_activity (
      ...
      v_org_id, 'updated', 'Updated subject enrollment data',
      jsonb_build_object(
        'enrolled_subject_count', jsonb_build_object('old', '0', 'new', '5'),
        'screen_failure_count', jsonb_build_object('old', '0', 'new', '2')
      ),
      v_profile_id, 'data.manager@trialetics.com', NOW() - INTERVAL '2 days'
    );
    ...
    INSERT INTO public.organization_activity (
      ...
      v_org_id, 'updated', 'Assigned Clinical Monitor',
      jsonb_build_object(
        'clinical_monitor', jsonb_build_object('old', 'N/A', 'new', 'David Brown')
      ),
      v_profile_id, 'john.admin@trialetics.com', NOW() - INTERVAL '1 day'
    );
    ...
    INSERT INTO public.organization_activity (
      ...
      v_org_id, 'updated', 'Updated site notes',
      jsonb_build_object(
      ...
      v_profile_id, v_creator_email, NOW() - INTERVAL '12 hours'
    );
    ...
    INSERT INTO public.organization_activity (
      ...
      v_org_id, 'updated', 'Updated website URL',
      jsonb_build_object(
        'website', jsonb_build_object('old', 'https://oldsite.org', 'new', 'https://mercygeneral.org')
      ),
      v_profile_id, 'admin@trialetics.com', NOW() - INTERVAL '6 hours'
    );
    ...
    INSERT INTO public.organization_activity (
      ...
      v_org_id, 'updated', 'Assigned Research Director',
      jsonb_build_object(
        'research_director', jsonb_build_object('old', 'N/A', 'new', 'James Thompson')
      ),
      v_profile_id, 'jane.monitor@trialetics.com', NOW() - INTERVAL '2 hours'
    );

  END LOOP;

  RAISE NOTICE 'Successfully seeded activity data for % organizations', v_org_count;
