-- Consume central-pool stock for a new site order by creating a per-order lot line.
-- Ledger entries remain on the source lot (shipped + received) so study/site metrics stay correct;
-- physical site quantity lives on the new lot's ip_lot_locations row so each ip_orders row maps to its own on-hand qty.
...
BEGIN
  ...
  -- Match ip_ship_to_site / ip_receive_at_site ledger snapshots on the source lot.
...
COMMENT ON FUNCTION public.ip_order_dispatch(UUID, UUID, UUID, UUID, INTEGER, TEXT, TEXT, TEXT, UUID) IS
  'Moves quantity from a central-pool lot into a new traced lot at a site; ledger shipped/received on source lot; order should reference the returned lot id.';

GRANT EXECUTE ON FUNCTION public.ip_order_dispatch(UUID, UUID, UUID, UUID, INTEGER, TEXT, TEXT, TEXT, UUID) TO authenticated;
