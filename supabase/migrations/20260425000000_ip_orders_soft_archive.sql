-- Soft archive for site-level orders (ip_orders.deleted_at).
-- Active UI lists exclude archived orders; restore clears deleted_at.
-- Site link archive guard counts only non-archived orders.

ALTER TABLE public.ip_orders
  ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ NULL;

COMMENT ON COLUMN public.ip_orders.deleted_at IS
  'When set, the order is hidden from default inventory summary; lot locations and ledger are unchanged.';

CREATE INDEX IF NOT EXISTS idx_ip_orders_study_item_site_active
  ON public.ip_orders (study_id, item_id, study_site_id)
  WHERE deleted_at IS NULL;

-- ---------------------------------------------------------------------------
-- ip_get_item_site_metrics: count only active (non-archived) orders
-- ---------------------------------------------------------------------------
...
'Per-site metrics for one catalog item. Pass p_include_archived true to list only archived site links; default false lists active links only. Order counts include only non-archived ip_orders.';

-- ---------------------------------------------------------------------------
-- ip_archive_item_site_link: block only on active orders
-- ---------------------------------------------------------------------------
