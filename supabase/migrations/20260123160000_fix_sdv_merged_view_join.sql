-- Fix the sdv_merged_view JOIN condition
-- The issue: JOIN was using both merge_key AND upload_id, but upload_id differs between tables
-- Solution: Join using merge_key and link through the primary_upload_id relationship
...
LEFT JOIN sdv_data_raw sdv 
  ...
  AND sdv.upload_id IN (
    SELECT sdv_upload_id 
    FROM sdv_uploads 
    WHERE id = site.upload_id 
    AND sdv_upload_id IS NOT NULL
  );
