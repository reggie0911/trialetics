-- =====================================================
-- SDV Merged View (replaces sdv_merged_records table)
-- =====================================================
-- This view joins sdv_site_data_raw and sdv_data_raw at query time
-- and performs all calculations on-the-fly
...
LEFT JOIN sdv_data_raw sdv ON site.merge_key = sdv.merge_key AND site.upload_id = sdv.upload_id;

-- Add comment
COMMENT ON VIEW sdv_merged_view IS 'Real-time view that merges Site Data Entry and SDV Data with calculated metrics';

-- Note: Views don't have RLS policies - they inherit from the underlying tables
-- The sdv_site_data_raw and sdv_data_raw tables already have RLS enabled
